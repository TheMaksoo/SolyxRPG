...
class LeaderboardService
{
     ...
     * loading every character and sorting in memory. 'guild_power' is a separate, guild-based board
     * entirely (see guildPowerBoard()), live-only, no range/scope support. */
    ...
    public function board(string $category, ?Character $character, string $range = 'all', string $scope = 'global'): array
    {
        ...
        $usesMemory = in_array($config['type'], ['power', 'trophies'], true) || $range !== 'all';
    ...
    /** 'power' and 'trophies' always route here (per-character PHP computation that can't be pushed into
     * SQL). Every other category routes here only when range != 'all', since a windowed value (current
     * minus a snapshot baseline) needs a per-character lookup that's simplest done in memory at this
     * game's scale — the same "load everything, sort in PHP" cost the whole endpoint used to pay for
     * every category before last session's SQL-pushdown pass. Trophies supports windowed ranges (rating
     * gained this week/day/season is a meaningful board) — Power doesn't, since gear/attribute Power isn't
     * a cumulative flow, so it always shows the live value regardless of range. */
    private function memoryBoard(string $category, array $config, ?Character $character, string $range, ?array $characterIds): array
    {
        $isPureLive = $config['type'] === 'power';
        $rangeApplies = $range !== 'all' && ! $isPureLive;
        // A windowed Trophies value is a rating DELTA, not an absolute rating — the tier badge (Bronze/
        // Diamond/etc.) only makes sense against the live, absolute number, so it's suppressed for any
        // range other than all-time.
        $showTrophyTier = $category === 'trophies' && $range === 'all';
    ...
    {
        if (in_array($config['type'], ['power', 'trophies'], true)) {
            $target = $this->valueFor($character, $category, $config);

            return Character::with($category === 'power' ? ['attributes_', 'inventory.item', 'pets.pet'] : ['pvpRecord'])
                ->get()
    ...
    public function fullRanking(string $category): Collection
    {
        ...
        if (in_array($config['type'], ['power', 'trophies'], true)) {
            return Character::with($category === 'power' ? ['attributes_', 'inventory.item', 'pets.pet', 'pvpRecord'] : ['pvpRecord'])
                ->get()
                ->map(fn (Character $c) => [
                    'character_id' => $c->id,
                    'value' => $category === 'trophies' ? ($c->pvpRecord->rating ?? 1000) : $c->effectiveStats()['power'],
                ])
                ->sortByDesc('value')
                ->values();
        }
    ...
    public function currentSeason(): LeaderboardSeason
    {
        ...
        return LeaderboardSeason::create([
            ...
            'name' => 'Season 1',
            'starts_at' => now(),
            // Calendar-month length, not a fixed 30 days — addMonthNoOverflow keeps a season that starts
            // on e.g. Jan 31 from overflowing into March.
